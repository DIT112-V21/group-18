...
class Main {
    ...
    companion object {
        ...
        fun main(args: Array<String?>?) {
            ...
            var pixelMap: Array<Array<RGB>> = map.getPixelMap()
        ...
        private fun detectCar(pixelMap: Array<Array<RGB>>): Car {
            ...
            for (i in pixelMap.indices) {
                for (j in 0 until pixelMap[0].length) {
                    val red: Int = RGB.correctedRGB(pixelMap[i][j].getRed())
                    val green: Int = RGB.correctedRGB(pixelMap[i][j].getGreen())
                    val blue: Int = RGB.correctedRGB(pixelMap[i][j].getBlue())
            ...
            starting = Coordinate((starting.getVerr() - length * firstBodyToTotalPixel) as Int, (starting.getHorr() - width * firstBodyToTotalPixel) as Int)
